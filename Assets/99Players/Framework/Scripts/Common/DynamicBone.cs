...
public class DynamicBone : MonoBehaviour {
    ...
    private void CheckDistance() {
        ...
        bool flag = (transform.position - base.transform.position).sqrMagnitude > m_DistanceToObject * m_DistanceToObject;
    ...
    private void UpdateParticles1(ParticleTree pt, float timeVar, int loopIndex) {
        ...
        Vector3 b = normalized * Mathf.Max(Vector3.Dot(pt.m_RestGravity, normalized), 0f);
        gravity -= b;
        gravity = (gravity + m_Force) * (m_ObjectScale * timeVar);
        Vector3 a = (loopIndex == 0) ? m_ObjectMove : Vector3.zero;
        for (int i = 0; i < pt.m_Particles.Count; i++) {
            Particle particle = pt.m_Particles[i];
            if (particle.m_ParentIndex >= 0) {
                Vector3 a2 = particle.m_Position - particle.m_PrevPosition;
                Vector3 b2 = a * particle.m_Inert;
                ...
                particle.m_Position += a2 * (1f - num) + gravity + b2;
            } else {
    ...
    private void UpdateParticles2(ParticleTree pt, float timeVar) {
        ...
        for (int i = 1; i < pt.m_Particles.Count; i++) {
            ...
            if (num2 > 0f || particle.m_Elasticity > 0f) {
                ...
                Vector3 a = (!particle.m_TransformNotNull) ? transformLocalToWorldMatrix.MultiplyPoint3x4(particle.m_EndOffset) : transformLocalToWorldMatrix.MultiplyPoint3x4(particle.m_TransformLocalPosition);
