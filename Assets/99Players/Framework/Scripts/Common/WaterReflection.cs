...
public class WaterReflection : MonoBehaviour {
    ...
    public void OnWillRenderObject() {
        ...
        for (int i = 0; i < array.Length; i++) {
            array[i].SetMatrix("_ProjMatrix", rhs);
        }
        if (m_DisablePixelLights) {
    ...
    private void CreateMirrorObjects(Camera currentCamera, out Camera reflectionCamera) {
        ...
        if (!m_ReflectionTexture || m_OldReflectionTextureSize != m_TextureSize) {
            ...
            m_ReflectionTexture.name = "__MirrorReflection" + GetInstanceID().ToString();
        ...
        if (!reflectionCamera) {
            GameObject gameObject = new GameObject("Mirror Refl Camera id" + GetInstanceID().ToString() + " for " + currentCamera.GetInstanceID().ToString(), typeof(Camera), typeof(Skybox));
