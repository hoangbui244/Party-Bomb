using System;
using UnityEngine;
public class GoldfishGetPointUi : MonoBehaviour
{
	...
	public void Show(int _point, Vector3 _worldPos)
	{
		...
		_worldPos = SingletonCustom<GoldfishGameManager>.Instance.GetCamera().WorldToScreenPoint(_worldPos);
		...
		float startZ = base.transform.position.y / 1000f;
		base.transform.SetLocalPositionZ(startZ);
		base.gameObject.SetActive(value: true);
		SetAlpha(1f);
		LeanTween.value(base.gameObject, 0f, 1f, 0.5f).setOnUpdate(delegate(float _value)
		{
			base.transform.SetLocalPositionZ(startZ + _value);
