...
using TMPro;
using UnityEngine;
public class WaterSpriderRace_UIManager : SingletonCustom<WaterSpriderRace_UIManager>
{
	[SerializeField]
	[Header("1人プレイ時のレイアウト")]
	private WaterSpriderRace_UserUILayoutData singleLayout;
	[SerializeField]
	[Header("2人プレイ時のレイアウト")]
	private WaterSpriderRace_UserUILayoutData twoPlayerLayout;
	[SerializeField]
	[Header("3人プレイ時のレイアウト")]
	private WaterSpriderRace_UserUILayoutData threePlayerLayout;
	[SerializeField]
	[Header("4人プレイ時のレイアウト")]
	private WaterSpriderRace_UserUILayoutData fourPlayerLayout;
	[SerializeField]
	[Header("画面フェ\u30fcド画像")]
	private SpriteRenderer screenFade;
	[SerializeField]
	[Header("画面分割用の枠線(1人用)")]
	private GameObject onePartition;
	[SerializeField]
	[Header("画面分割用の枠線(2人用)")]
	private GameObject twoPartition;
	[SerializeField]
	[Header("画面分割用の枠線(3人用)")]
	private GameObject threePartition;
	[SerializeField]
	[Header("画面分割用の枠線(4人用)")]
	...
	[SerializeField]
	[Header("移動量(ロ\u30fcカライズ位置変更用(ひとりプレイ))")]
	private Vector3 movePos;
	[SerializeField]
	[Header("移動量(ロ\u30fcカライズ位置変更用(マルチプレイ))")]
	private Vector3 movePos_multi;
	...
	public void Init()
	{
		...
		onePartition.SetActive(value: false);
		twoPartition.SetActive(value: false);
		threePartition.SetActive(value: false);
		...
		case 1:
			if (!(singleLayout == null))
			{
				...
				singleLayout.Init();
				onePartition.SetActive(value: true);
			}
			break;
		case 2:
		...
		case 3:
			if (!(threePlayerLayout == null))
			{
				...
				threePartition.SetActive(value: true);
			}
			break;
		case 4:
		...
		for (int i = 0; i < ControlOperationAnchor.Length; i++)
		{
			...
			case 1:
				...
				break;
			case 2:
				ControlOperationAnchor[i].transform.position += movePos_multi;
				break;
			case 3:
				ControlOperationAnchor[i].transform.position += movePos_multi;
				break;
	...
	{
		if (WaterSpriderRace_Define.GM.IsDuringGame() && !isRankUpdateFlg)
		{
	...
	public void SetUserUIData(WaterSpriderRace_PlayerManager.UserData[] _userDatas)
	{
		WaterSpriderRace_Define.UserType[] array = new WaterSpriderRace_Define.UserType[WaterSpriderRace_Define.MEMBER_NUM];
	...
	public void SetScore(WaterSpriderRace_Define.UserType _userType, int _score)
	{
	}
