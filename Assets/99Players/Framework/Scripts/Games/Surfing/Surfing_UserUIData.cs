...
using TMPro;
using UnityEngine;
public class Surfing_UserUIData : MonoBehaviour
{
	[Serializable]
	...
	[SerializeField]
	[Header("ポイント獲得処理")]
	private Surfing_GetPointUI getPointUI;
	[SerializeField]
	[Header("フェ\u30fcド用画像")]
	...
	public void GetPointUI(int _point)
	{
		getPointUI.GetPoint(_point);
	}
	public void Fade(bool _set = true)
	{
		UnityEngine.Debug.Log("fade処理!!");
		LeanTween.cancel(id);
		if (_set)
		{
			id = LeanTween.alpha(spFade.gameObject, 1f, 1f).id;
		}
		else
		{
			id = LeanTween.alpha(spFade.gameObject, 0f, 1f).id;
		}
	}
	private void SetPlayerIcon()
	{
		if (!(playerIconSp == null))
		{
			switch (userType)
			{
			case Surfing_Define.UserType.PLAYER_1:
