...
using TMPro;
using UnityEngine;
public class LegendarySword_UserUIData : MonoBehaviour
{
	[Serializable]
	...
	public void Init(LegendarySword_Define.UserType _userType)
	{
		userType = _userType;
		SetPlayerIcon();
		SetCharacterIcon();
	...
	private void SetPlayerIcon()
	{
		if (!(playerIconSp == null))
		{
			switch (userType)
			{
			case LegendarySword_Define.UserType.PLAYER_1:
