...
using UnityEngine;
public class GS_CharacterSelectLayoutThree : GS_CharacterSelectLayoutBase
{
	private enum State
	{
		SELECT_WAIT,
	...
	public void Show()
	{
		...
		for (int num = 0; num < arraySelectIdx.Length; num++)
		{
			arraySelectIdx[num] = -1;
		}
		for (int num2 = 0; num2 < arrayCursorManager.Length; num2++)
		{
			switch (num2)
			{
			case 0:
				for (int num4 = 0; num4 < spCharacter1p.Length; num4++)
				{
					spCharacter1p[num4].SetActive(num4 == arrayCursorManager[num2].GetSelectNo());
				}
				break;
			case 1:
				for (int num5 = 0; num5 < spCharacter2p.Length; num5++)
				{
					spCharacter2p[num5].SetActive(num5 == arrayCursorManager[num2].GetSelectNo());
				}
				break;
			case 2:
				for (int num3 = 0; num3 < spCharacter3p.Length; num3++)
				{
					spCharacter3p[num3].SetActive(num3 == arrayCursorManager[num2].GetSelectNo());
				}
				break;
			}
			UpdateNamePlate(num2);
		}
		for (int num6 = 0; num6 < arrayCursorManager.Length; num6++)
		{
			arrayPreviewChara[num6].SetGameStyle(GS_Define.GameType.BOMB_ROULETTE, arrayCursorManager[num6].GetSelectNo());
		}
	...
	private void UpdateNamePlate(int _updateIdx)
	{
		...
		case 0:
			...
			break;
		case 1:
			for (int k = 0; k < spCharacter2p.Length; k++)
			{
				spCharacter2p[k].transform.localPosition = arrayThumbPos2p[k];
				spCharacter2p[k].SetActive(k == arrayCursorManager[_updateIdx].GetSelectNo());
			}
			break;
		case 2:
			for (int i = 0; i < spCharacter3p.Length; i++)
			{
				spCharacter3p[i].transform.localPosition = arrayThumbPos3p[i];
				spCharacter3p[i].SetActive(i == arrayCursorManager[_updateIdx].GetSelectNo());
			}
			break;
		}
