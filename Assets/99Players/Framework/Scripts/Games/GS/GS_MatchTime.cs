using GamepadInput;
using UnityEngine;
public class GS_MatchTime : MonoBehaviour
{
	[SerializeField]
	[Header("選択フレ\u30fcム")]
	...
	public void Set(GS_Define.GameType _type)
	{
		...
		SetFocus(SingletonCustom<GameSettingManager>.Instance.SelectMatchTimeIdx);
	}
	public void Close()
	{
	...
	private void Update()
	{
		...
		{
			SingletonCustom<GameSettingManager>.Instance.SelectMatchTimeIdx = (SingletonCustom<GameSettingManager>.Instance.SelectMatchTimeIdx + 1) % arraySelectFrame.Length;
			SetFocus(SingletonCustom<GameSettingManager>.Instance.SelectMatchTimeIdx);
