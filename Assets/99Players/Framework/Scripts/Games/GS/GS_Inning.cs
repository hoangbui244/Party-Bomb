using GamepadInput;
using UnityEngine;
public class GS_Inning : MonoBehaviour
{
	[SerializeField]
	[Header("選択フレ\u30fcム")]
	...
	public void Set(GS_Define.GameType _type)
	{
		...
		SetFocus(SingletonCustom<GameSettingManager>.Instance.SelectInningNum);
	}
	public void Close()
	{
	...
	private void Update()
	{
		...
		{
			SingletonCustom<GameSettingManager>.Instance.SelectInningNum = (SingletonCustom<GameSettingManager>.Instance.SelectInningNum + 1) % arraySelectFrame.Length;
			SetFocus(SingletonCustom<GameSettingManager>.Instance.SelectInningNum);
