using UnityEngine;
public class AlpineSkiing_UserUILayoutData : MonoBehaviour {
    [SerializeField]
    [Header("ゲ\u30fcム時間")]
    private CommonGameTimeUI_Font_Time[] commonGameTime;
    [SerializeField]
    [Header("ユ\u30fcザ\u30fcUIのデ\u30fcタ")]
    private AlpineSkiing_UserUIData[] userUIDatas;
    [SerializeField]
    [Header("～組目の画像")]
    ...
    public void SetUserUIData(AlpineSkiing_Define.UserType[] _userTypeArray) {
        for (int i = 0; i < userUIDatas.Length; i++) {
            userUIDatas[i].Init(_userTypeArray[i]);
        }
    }
    public void SetUserScore(AlpineSkiing_Define.UserType _userType, int _score) {
    }
    public void SetUserRanking() {
        for (int i = 0; i < userUIDatas.Length; i++) {
            if (AlpineSkiing_Define.PM.Players[i].SkiBoard.processType != AlpineSkiing_SkiBoard.SkiBoardProcessType.GOAL) {
                userUIDatas[i].SetRanking(SingletonCustom<AlpineSkiing_RankingManager>.Instance.playerRanking[i]);
                continue;
            }
            goalRankingCnt = 1;
            for (int j = 0; j < AlpineSkiing_Define.PM.UserData_Group1.Length; j++) {
                if (AlpineSkiing_Define.PM.UserData_Group1[i].goalTime > AlpineSkiing_Define.PM.UserData_Group1[j].goalTime) {
                    goalRankingCnt++;
                }
            }
    ...
    public void SetGroupNumberIcon() {
        if (!(groupNumberSp == null)) {
            ...
            if (AlpineSkiing_Define.PM.CheckNowGroup1Playing()) {
