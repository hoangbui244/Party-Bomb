...
namespace UnityStandardAssets.Utility
{
	public class WaypointCircuit : MonoBehaviour
	{
		...
		[SerializeField]
		private Color gizmoColorSelected = Color.yellow;

		[SerializeField]
		private Color gizmoColor = new Color(1f, 0.92f, 0f, 0.5f);
		...
		[SerializeField]
		[Header("デバッグ用：指定距離の位置にSphereを表示するかどうか")]
		private bool isDebugSphere;
		...
		public float getViewLength => ViewLength;
		...
		private void OnDrawGizmos()
		{
			...
			if (isDebugSphere)
			{
				Gizmos.DrawWireSphere(GetRoutePosition(debugDis), 2f);
			}
		}
		...
		private void DrawGizmos(bool selected)
		{
			...
			CachePositionsAndDistances();
			Length = distances[distances.Length - 1];
			Gizmos.color = (selected ? gizmoColorSelected : gizmoColor);
