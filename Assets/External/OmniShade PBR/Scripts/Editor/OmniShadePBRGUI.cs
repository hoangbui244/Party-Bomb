...
#endif
{
	...
	List<string> stripDisplayNamePrefixes = new List<string>() 
	{
		"Height Colors",
		"Shadow Overlay",
		"Plant Sway",
    ...
    int forceExpand = 1;
	Dictionary<string, bool> headerOpenDic = new Dictionary<string, bool>();
	Dictionary<MaterialProperty, int> propOrder = new Dictionary<MaterialProperty, int>();
...
#endif
	{
		...
		{
			OmniShadePBRGUI.AutoEnableShaderKeywords(material);
			if (material.shader.name.ToLower().Contains("transparent"))
				isTransparentShader = true;
		}
    }

	public static void AutoEnableShaderKeywords(Material mat) {
		// HDRP material fixes
#if UNITY_PIPELINE_HDRP
		...
		{
			if (!mat.IsKeywordEnabled("CAMERA_FADE_ON"))
				mat.EnableKeyword("CAMERA_FADE_ON");
		}
		else
		{
			if (mat.IsKeywordEnabled("CAMERA_FADE_ON"))
				mat.DisableKeyword("CAMERA_FADE_ON");
		}
    ...
    void RenderGUI(MaterialEditor materialEditor, MaterialProperty[] properties) {
		...
		// Fetch header info
		this.FetchHeaderGroups(materialEditor);
    ...
    void FetchHeaderGroups(MaterialEditor materialEditor) {
		var mat = materialEditor.target as Material;
		...
		for (int i = 0; i < numProps; i++) {
			...
			switch (this.forceExpand) {
				...
				case 2:  // Collapse
					...
					break;  
				default: break;  				// -1 Keep Existng
			}            

			// Cache
			if (!this.headerOpenDic.ContainsKey(headerName))
				this.headerOpenDic.Add(headerName, isInUse);
			if (this.forceExpand == 0 || this.forceExpand == 1)  // Expand All or Expand Active
				this.headerOpenDic[headerName] |= isInUse;
			else if (this.forceExpand == 2)  // Collapse
				this.headerOpenDic[headerName] = isInUse;
		}
        this.forceExpand = -1;
	...
	bool IsKeyProperty(string propDesc, ShaderPropertyType propType) {
		return ((propType == ShaderPropertyType.Float && propDesc.StartsWith("Enable")) || 
			...
			propDesc == "Metallic" ||
			propDesc == "Emission Color" ||
			propDesc == "Camera Fade Start Distance");
	}
	...
	string GetHeaderName(string propDesc) {
		foreach (var header in this.headers) {
			if (!this.isTransparentShader && header == "Camera Fade")
				continue;
